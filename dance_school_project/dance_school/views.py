...
def home(request):
    ...
    # Форма расчета скидки (для клиентов) - ИСПОЛЬЗУЕТ ФУНКЦИЮ БД
    if request.method == 'POST' and 'discount_form' in request.POST:
        if user_info and user_info.get('role') == 'client':
            try:
                ...
                # ВЫЗОВ ФУНКЦИИ БД для расчета скидки
                ...
                discount_result = {
                    ...
                    'note': 'Расчет выполнен функцией БД'
                }
            except Exception as e:
                discount_result = {'error': str(e)}
    
    # Получаем расписание через ПРЕДСТАВЛЕНИЕ БД
    try:
        ...
        print(f"Ошибка получения расписания из представления: {e}")
        # Fallback на старый метод
        try:
            schedule_data = DatabaseService.execute_query("""
                SELECT s.schedule_id, t.trainer_full_name_nesterovas_21_8, 
                       d.dance_style_name_nesterovas_21_8, s.class_weekday_nesterovas_21_8,
                       TO_CHAR(s.class_start_time_nesterovas_21_8, 'HH24:MI'), 
                       TO_CHAR(ts.class_end_time_nesterovas_21_8, 'HH24:MI'),
                       s.hall_number_nesterovas_21_8, h.hall_capacity_nesterovas_21_8,
                       s.subscription_price_nesterovas_21_8, s.schedule_status_nesterovas_21_8,
                       COALESCE(reg.registration_count, 0),
                       COALESCE(h.hall_capacity_nesterovas_21_8, 0) - COALESCE(reg.registration_count, 0)
    ...
    try:
        ...
        # ВЫЗОВ СКАЛЯРНОЙ ФУНКЦИИ БД
        longest_client_name = DatabaseService.get_longest_client_name()
    except Exception as e:
        print(f"Ошибка статистики: {e}")
        total_classes = 0
        total_trainers = 0
        total_styles = 0
        longest_client_name = "Не доступно"
    ...
    context = {
        ...
        'longest_client_name': longest_client_name,  # НОВОЕ ПОЛЕ
...
def admin_dashboard(request):
    ...
    try:
        ...
        # ВЫЗОВ ТАБЛИЧНОЙ ФУНКЦИИ БД
        top10_expensive_schedules = DatabaseService.get_top10_expensive_schedules()
        ...
        context = {
            ...
            # ДАННЫЕ ИЗ ФУНКЦИЙ БД
            'top10_expensive_schedules': top10_expensive_schedules,
            'longest_client_name': longest_client_name,
            # Статистика
